...
<br><hr><br>
<form id="delete_all">
  <input type="text" name="action" placeholder="action: delete_all" value="delete_all"><br>
  <input type="text" name="user_idx" placeholder="user_idx"><br>
  ...
  function delete_all() {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status === 'success') {
          // 결과 메시지 표시
          result_div.innerHTML = '<p>${response.status}</p><p>${response.message}</p><p>이미지 전체 삭제 완료</p>';
        } else {
          result_div.innerHTML = `<p>${response.status}</p><p>${response.message}</p>`;
        }
      },
